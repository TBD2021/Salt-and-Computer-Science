...
树(Tree)是一种抽象数据类型，它是由n（n>0）个有限节点组成一个具有层次关系的集合，具有以下特点：
...
二叉树的深度优先遍历用栈来实现，可以分为先序遍历、中序遍历和后序遍历。可以用递归、非递归两种方式实现二叉树的先序、中序、后序遍历。非递归方式实现遍历就是自己手动压栈而已。

- 先序遍历：头节点 - 左子树上所有的节点 - 右子树上所有的节点
- 中序遍历：左子树上所有的节点 - 头节点 - 右子树上所有的节点
- 后序遍历：左子树上所有的节点 - 右子树上所有的节点 - 头节点
