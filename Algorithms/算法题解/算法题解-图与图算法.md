...
一个项目由很多个任务组成，而这些任务中，某些任务是其他任务可以执行的先决条件，也就是任务之间的存在依赖关系。如果能够在给定的依赖关系下完成全部任务，则返回一个（或全部）的满足依赖关系的线性可执行序列；否则，返回空序列。

**核心思路**

根据项目的构建一个有向图。其中，每个节点表示一个独立的任务，节点之间的有向边表示任务之间的先后依赖关系，从节点A指向节点B的边表示任务A是任务B的先决条件。
